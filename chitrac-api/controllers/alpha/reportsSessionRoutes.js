...
router.get("/analytics/item-sessions-summary", async (req, res) => {
  ...
  // Operator hybrid helpers imported from reportFunctions
  ...
  router.get("/analytics/item-sessions-summary-cache", async (req, res) => {
    try {
      ...
      if (useHybrid) {
        ...
        if (completeDays.length > 0) {
          itemTotals = await getItemCachedDataForDays(db,completeDays);
        }
        
        // Get data from sessions for partial days
        if (partialDays.length > 0) {
          sessionData = await getItemSessionDataForPartialDays(db,partialDays);
        }
