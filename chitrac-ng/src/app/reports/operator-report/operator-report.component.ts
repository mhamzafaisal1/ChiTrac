...
export class OperatorReportComponent implements OnInit, OnDestroy {
  ...
  gridComponents = [
    MachineStatusStackedChartComponent, // Operator Status Stacked Bar
    RankedOeeChartComponent, // Ranked OEE% by Operator
    ItemStackedChartComponent, // Item Stacked Bar by Operator
    FaultStackedBarByMachineComponent  // Fault Stacked Bar by Operator
  ];
  ...
  private processChartData(data: any): void {
    if (data.charts) {
      ...
      this.chartDataArray = [
        ...
        }, // Operator Status Stacked Bar
        {
          ...data.charts.efficiencyRanked,
          legend: { show: true, position: 'right' }
        }, // Ranked OEE% by Operator
        {
          ...data.charts.itemsStacked,
          legend: { show: true, position: 'right' }
        }, // Item Stacked Bar by Operator
        {
          ...data.charts.faultsStacked,
          legend: { show: true, position: 'right' }
        } // Fault Stacked Bar by Operator
      ];
    }
  }
  ...
  async downloadOperatorSummaryPdf(): Promise<void> {
    ...
    try {
      ...
      let y = margin;
        ...
        doc.setFontSize(14);
      doc.text('OPERATOR REPORT', margin, y); y += 18;
      doc.setFontSize(10);
