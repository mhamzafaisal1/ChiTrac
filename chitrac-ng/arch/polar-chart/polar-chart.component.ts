...
export class PolarChartComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      this.chartContainer &&
      (changes['data'] || changes['title'] || changes['levels'] || changes['maxValue'])
    ) {
      this.renderChart();
    }
  }
  ...
  private renderChart(): void {
    ...
    const height = Math.max(360, Math.min(width * 0.78, 560));
    const margin = { top: 54, right: 88, bottom: 52, left: 88 };
    const innerWidth = width - margin.left - margin.right;
    const innerHeight = height - margin.top - margin.bottom;
    const radius = Math.max(90, Math.min(innerWidth, innerHeight) / 2);
    const centerX = width / 2;
    const centerY = margin.top + innerHeight / 2;
    const levelCount = Math.max(1, Math.floor(this.levels));

    const textColor = this.isDarkTheme ? '#f3f4f6' : '#1f2937';
    const mutedTextColor = this.isDarkTheme ? '#aeb7c4' : '#64748b';
    const gridColor = this.isDarkTheme ? 'rgba(226, 232, 240, 0.22)' : 'rgba(100, 116, 139, 0.28)';
    const axisColor = this.isDarkTheme ? 'rgba(226, 232, 240, 0.38)' : 'rgba(71, 85, 105, 0.34)';
    ...
    series.forEach((item, seriesIndex) => {
      ...
      chart.append('path')
        ...
        .attr('d', line)
        .attr('fill', color)
        .attr('fill-opacity', series.length === 1 ? 0.24 : 0.16)
        .attr('stroke', color)
